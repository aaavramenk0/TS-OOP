// Generics (Dynamic)
export class User<T> { // <T> is the type of the class
    name: string;
    age: number;
    email: string;
    isMale: boolean;
...
user1.mergeClassicUser({ name: { first: "Oleksandr", last: "Avramenko" } });

const user2 = new User<ClassicUser2>();
user2.mergeClassicUser({ name: { first: "Oleksandr", middle: "none", last: "Avramenko" } });